...
def user_manage_trips():
	uid=session['user_id']
	data={}
	z="select * from category"
	res1=select(z)
	data['category']=res1
	q="SELECT * FROM trips inner join category using(category_id) where User_id='%s'"%(uid)
	res=select(q)
	print(res)
	data['user']=res
...
def user_view_trips():
	...
	if 'submit' in request.form:
		...
		sr="%"+search+"%"
		q="select *,trips.place as tplace from `trips` where `place` like '%s'"%(sr)
		res=select(q)
		data['res']=res

	q="SELECT *,trips.place as tplace FROM trips inner join userregistration using(user_id) inner join category using(category_id) where user_id!='%s'"%(uid)
	res=select(q)
	print(res)
	data['user']=res
	if 'action' in request.args:
		...
		action=None
	if action=='request':
		q="insert into request values(null,'%s','%s','pending',NOW(),'%s')"%(tid,uid,cid)
		insert(q)
...
def user_view_trips_request():
	...
	data={}
	# data['tid']=tid
	q="SELECT *,`trips`.`Place` AS tplace FROM request INNER JOIN trips USING(trips_id) INNER JOIN category ON category.`category_id`=trips.`category_id` INNER JOIN userregistration ON userregistration.`user_id`=request.`User_id` WHERE request.Trips_id='%s' AND trips.User_id='%s'"%(tid,uid)
	print(q)
	res=select(q)
	data['user']=res
	...
	if action=="reject":
		...
		update(q)
		return redirect(url_for('user.user_view_trips_request'))
	return render_template('user_view_trips_request.html',data=data)

@user.route('/user_view_req',methods=['get','post'])
def user_view_req():
	data={}
	uid=session['user_id']
	cid=request.args['cid']
	data['cid']=cid
	q="SELECT * FROM request INNER JOIN trips USING(Trips_id) WHERE trips.category_id='%s' AND `trips`.`Trips_id` IN(SELECT `Trips_id` FROM `trips` WHERE User_id='%s')"%(cid,uid)
	print(q)
	res=select(q)
	data['req']=res
	if 'action' in request.args:
	...
	if action=="reject":
		q="update request set Status='reject' where Request_id='%s'"%(rid)
		update(q)	
	return render_template('user_view_req.html',data=data,cid=cid)
...
def user_view_req_status():
	...
	q="SELECT *,`trips`.`Place` AS pl FROM request INNER JOIN userregistration ON request.User_id=userregistration.user_id INNER JOIN category USING(category_id) INNER JOIN `trips` ON `trips`.`Trips_id`=`request`.`Trips_id` where request.User_id='%s'"%(uid)
	print(q)
	res=select(q)
	data['rqt']=res
	print(res)
...
def user_chat():
	...
	data['uid']=uid
	rid=request.args['rid']
	q="select * from chat inner join userregistration on sender_id=user_id where sender_id='%s'"%(uid)
	data['chat']=select(q)
	print(q)
	print(data['chat'])
	q="select * from userregistration where user_id='%s'"%(uid)
	res=select(q)
	data['opuname']=res[0]['Firstname']
	if 'send' in request.form:
		msg=request.form['msg']
		q="insert into chat values(NULL,'%s','%s','%s',NOW())"%(uid,rid,msg)
		insert(q)
		return redirect(url_for('user.user_chat',rid=rid))
...
def user_view_chatted_users():
	...
	q="select * from chat where Receiver='%s' "%(Request_id)
	print(q)
	res=select(q)
	print(res)
	data['user']=res
	...
	# res=select(q)
	# data['opuname']=res[0]['Firstname']
	if 'csubmit' in request.form:
		msg=request.form['msg']
		q="insert into chat values(NULL,'%s','%s','%s',NOW())"%(uid,Request_id,msg)
		insert(q)
		return redirect(url_for('user.user_view_chatted_users',Request_id=Request_id))
