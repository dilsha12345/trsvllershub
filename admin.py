...
def adminviewtrip():
	data={}
	q="select *,trips.place as tplace from trips inner join category using(category_id) inner join userregistration using(user_id)"
	res=select(q)
	data['user']=res
...
def admin_view_request():
	data={}
	id=request.args['id']
	q="SELECT *,trips.place as tplace FROM request INNER JOIN trips ON `request`.`Trips_id`=`trips`.`Trips_id` INNER JOIN userregistration ON `request`.`User_id`=`userregistration`.`user_id` INNER JOIN category ON `category`.`category_id`=`request`.`category_id` where `request`.Trips_id='%s'"%(id)
	print(q)
	res=select(q)
	print(res)
	data['rqt']=res
